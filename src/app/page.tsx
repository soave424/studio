...
export default function TeamWeaverPage() {
  ...
  const handleAnalyze = () => {
    ...
    try {
      ...
      console.error(e);
      setError(
        e instanceof Error ? e.message : "데이터 분석 중 오류가 발생했습니다."
      );
    }
  };
  ...
  const handleGenerateTeams = () => {
    setError(null);
    try {
      const hasPredefinedTeams = students.some(
        (s) => s.teamId && s.teamId.trim() !== ""
      );
      ...
      if (hasPredefinedTeams) {
        ...
        Object.keys(studentsByLevel).forEach((level) => {
          newTeamsByLevel[level] = createBalancedTeams(
      ...
      console.error(e);
      setError(
        e instanceof Error ? e.message : "조 편성 중 오류가 발생했습니다."
      );
    }
  };
  ...
  const handleDragStart = (
    ...
    const dragData: DraggedItem = {
      student,
      source: { level: sourceLevel, teamIndex: sourceTeamIndex },
    };
  ...
  const handleDrop = (destLevel: string, destTeamIndex: number) => {
    ...
    setTeamsByLevel((prev) => {
      ...
      const studentIndex = sourceTeam.findIndex(
  ...
  const handleAiModalOpen = async (team: Student[]) => {
    ...
    const teamMembersForAI = team.map(({ name, school, level }) => ({
      name,
      school,
      level,
    }));
  ...
  const summary = useMemo(() => {
    ...
    const regionCounts = allStudents.reduce((acc, s) => {
      acc[s.region] = (acc[s.region] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
    const schoolCounts = allStudents.reduce((acc, s) => {
    ...
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 max-w-7xl">
      <header className="text-center mb-8">
        ...
        <p className="mt-2 text-lg text-muted-foreground">
          ✨ AI로 스마트하게 조를 편성하고 완벽한 팀워크를 만드세요.
        </p>
      </header>
      ...
      {appState === "input" && (
        <Card className="w-full max-w-4xl mx-auto shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              엑셀이나 명단 정보를 그대로 복사해서 붙여넣으세요. 이전에 내보낸
              파일을 붙여넣으면 조 편성이 복원됩니다.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="member-data" className="text-base">
                참가자 정보 붙여넣기
              </Label>
              <Textarea
                id="member-data"
          ...
          <CardFooter className="flex-col sm:flex-row gap-2">
            <Button
              ...
              className="w-full sm:w-auto text-lg py-6 flex-grow"
            >
              <Wand2 className="mr-2" /> 데이터 분석 및 확인
            </Button>
            <Button
              onClick={() => setRawText(presetData)}
              variant="outline"
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                AI가 분석한 데이터입니다. 잘못된 정보가 있다면 표에서 직접
                수정해주세요.
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <TableRow key={s.id + i}>
                        <TableCell>
                          <Input
                            value={s.teamId}
                            onChange={(e) =>
                              handleStudentChange(i, "teamId", e.target.value)
                            }
                            placeholder="미정"
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                            value={s.id}
                            onChange={(e) =>
                              handleStudentChange(i, "id", e.target.value)
                            }
                          />
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Input
                            value={s.gender}
                            onChange={(e) =>
                              handleStudentChange(i, "gender", e.target.value)
                            }
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                            value={s.school}
                            onChange={(e) =>
                              handleStudentChange(i, "school", e.target.value)
                            }
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                            value={s.level}
                            onChange={(e) =>
                              handleStudentChange(i, "level", e.target.value)
                            }
                          />
                        </TableCell>
                        <TableCell>
                          <Input
                            value={s.region}
                            onChange={(e) =>
                              handleStudentChange(i, "region", e.target.value)
                            }
                          />
                        </TableCell>
              ...
              <Button
                ...
                className="mt-4"
              >
                <PlusCircle className="mr-2 h-4 w-4" /> 인원 추가하기
              </Button>
            </CardContent>
          </Card>
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleGenerateTeams} className="text-lg py-6">
              이 정보로 조 편성 시작!
            </Button>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="flex flex-wrap gap-4 justify-between items-center">
            ...
            <div className="flex gap-2 flex-wrap">
              <Button onClick={() => setAppState("review")} variant="secondary">
                편성 수정하기
              </Button>
              <Button onClick={() => exportTeamsToCSV(teamsByLevel)}>
                <FileDown className="mr-2 h-4 w-4" />
                결과 내보내기
              </Button>
              <Button onClick={() => setSeatingChartModalOpen(true)}>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <Card className="bg-muted/50">
                ...
                <CardContent className="text-base space-y-1">
                  <p>
                    총: <span className="font-bold">{summary.total}</span>명
                  </p>
                  <p>
                    남자:{" "}
              ...
              <Card className="bg-muted/50">
                ...
                <CardContent className="text-base space-y-1">
                  {summary.sortedLevels.map((level) => (
                    <p key={level}>
                      {level}:{" "}
              ...
              <Card className="bg-muted/50">
                ...
                <CardContent className="text-sm space-y-1 max-h-48 overflow-y-auto">
                  {Object.keys(summary.regionCounts)
                    .sort()
                    .map((region) => (
                      <p key={region}>
                        {region}:{" "}
                        <span className="font-bold">
                          {summary.regionCounts[region] || 0}
                        </span>
                        명
                      </p>
                    ))}
              ...
              <Card className="bg-muted/50 md:col-span-2 lg:col-span-3">
                ...
                <CardContent className="text-sm flex flex-wrap gap-x-4 gap-y-1 max-h-48 overflow-y-auto">
                  {Object.keys(summary.schoolCounts)
                    .sort()
                    .map((school) => (
                      <p key={school}>
                        {school}:{" "}
                        <span className="font-bold">
                          {summary.schoolCounts[school] || 0}
                        </span>
                        명
                      </p>
                    ))}
          ...
          {Object.keys(teamsByLevel)
            .sort(
              (a, b) =>
                ["초등", "중등", "고등", "대학", "기타"].indexOf(a) -
                ["초등", "중등", "고등", "대학", "기타"].indexOf(b)
            )
            .map((level) => (
              <div key={level}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      ...
                      >
                        <CardHeader className="flex flex-row items-center justify-between">
                          <div>
                            <CardTitle>
                              조{" "}
                              {Object.values(teamsByLevel)
                                .flat()
                                .indexOf(team) + 1}
                            </CardTitle>
                            <CardDescription>
                              총 {team.length}명 (남:{maleCount}/여:
                              {femaleCount})
                            </CardDescription>
                          </div>
                          {team.length === 0 && (
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleDeleteTeam(level, teamIndex)}
                            >
                              <Trash2 className="h-4 w-4 text-destructive" />
                            </Button>
                          )}
                          ...
                          <ul className="space-y-2">
                            {team.map((student) => (
                              <li
                                ...
                                draggable
                                onDragStart={(e) =>
                                  handleDragStart(e, student, level, teamIndex)
                                }
                        ...
                        <CardFooter>
                          <Button
                            onClick={() => handleAiModalOpen(team)}
                            className="w-full"
                  ...
                  <Button
                    variant="outline"
                    className="h-full min-h-48 border-dashed border-2 flex-col gap-2"
                    onClick={() => handleAddTeam(level)}
                  >
            ...
            <div className="space-y-6 mt-4">
              ...
              <div>
                ...
                <ul className="list-disc list-inside mt-2 space-y-2 text-base">
                  {aiSuggestion.icebreakers.map((q, i) => (
                    <li key={i}>{q}</li>
                  ))}
                </ul>
              </div>
      ...
      <Dialog
        open={isSeatingChartModalOpen}
        onOpenChange={setSeatingChartModalOpen}
      >
